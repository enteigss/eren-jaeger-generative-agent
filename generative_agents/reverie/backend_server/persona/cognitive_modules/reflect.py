    ...
    for i in persona.a_mem.seq_event + persona.a_mem.seq_thought:
        if "idle" not in i.embedding_key:
            ...
            if isinstance(last_accessed, str):
                try:
                    last_accessed = datetime.datetime.strptime(last_accessed, "%B %d, %Y, %H:%M:%S")
                except ValueError:
                    # If the string format doesn't match, use current time as fallback
                    last_accessed = datetime.datetime.now()
            nodes.append([last_accessed, i])
            string_timestamps = [(i, node[0]) for i, node in enumerate(nodes) if isinstance(node[0], str)]
  else:
...
def generate_insights_and_evidence(persona, nodes, n=5): 
  ...
  ret = run_gpt_prompt_insight_and_guidance(persona, statements, n, verbose=True)[0]
  try: 
