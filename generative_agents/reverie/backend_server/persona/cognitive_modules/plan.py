...
def generate_convo(maze, init_persona, target_persona): 
  logging.debug("DEBUG:: Entering generate_convo")
...
def _long_term_planning(persona, new_day): 
  ...
  print(f"New day status: {new_day}")
  ...
  if new_day == "First day": 
    print("Generating first daily plan")
  ...
  # add up to 24 hours.
  print("Generating hourly schedule")
  ...
  print(f"Schedule length after planning: {len(persona.scratch.f_daily_schedule)}")
...
def _determine_action(persona, maze): 
  ...
  print("\n=== Determine Action ===")
  print(f"Persona: {persona.name}")
  print(f"Schedule length: {len(persona.scratch.f_daily_schedule)}")
...
def _should_react(persona, retrieved, personas): 
  ...
  """
  print("DEBUG:: Entering should react")
  ...
  def lets_react(init_persona, target_persona, retrieved): 
    ...
    print("DEBUG:: React mode:", react_mode)
  ...
  if persona.scratch.chatting_with: 
    print("DEBUG:: Persona is chatting, returning False")
    return False
  if "<waiting>" in persona.scratch.act_address: 
    print("DEBUG:: Persona is waiting, returning False")
    return False
  ...
  if ":" not in curr_event.subject: 
    ...
    react_mode = lets_react(persona, personas[curr_event.subject], 
                            retrieved)
    return react_mode
  print("DEBUG:: Not a persona event, returning False")
  return False
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
  ...
  logging.debug("DEBUG:: Entering _chat_react")
...
def plan(persona, maze, personas, new_day, retrieved): 
  ...
  if focused_event: 
    ...
    print("DEBUG:: Got reaction mode:", reaction_mode)
    if reaction_mode: 
      # If we do want to chat, then we generate conversation 
      if reaction_mode[:9] == "chat with":
        print("DEBUG:: Chatting with")
