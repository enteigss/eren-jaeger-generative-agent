...
def run_gpt_prompt_daily_plan(persona, 
  ...
  print ("PROMPT TEMPLATE: daily_planning_v6.txt")
...
def run_gpt_prompt_task_decomp(persona, 
                               ...
                               test_input=None, 
                               verbose=False): 
  # Adding logs to debug TOKEN LIMIT EXCEEDED bug
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    try:
        ...
        for count, i in enumerate(temp):
            if count != 0:
                cleaned = " ".join([j.strip() for j in i.split(" ")][3:])
        ...
        for count, i in enumerate(_cr):
            ...
            print(f"Split on duration: {k}")
            task = k[0]
            duration = int(k[1].split(",")[0].strip())
            print(f"Extracted - Task: {task}, Duration: {duration}")
        ...
        for count, i in enumerate(_cr): 
          k = [j.strip() for j in i.split("(duration in minutes:")]
          i_task = k[0]
          i_duration = int(k[1].split(",")[0].strip())
          print(f"i_duration: {i_duration} ({type(i_duration)})")
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  print ("PROMPT TEMPLATE: decide_to_talk_v2.txt")
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved,test_input=None, 
  ...
  print ("PROMPT TEMPLATE: decide_to_react_v1.txt")
  prompt_input = create_prompt_input(persona, target_persona, retrieved,
                                     test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  print ("PROMPT TEMPLATE: create_conversation_v2.txt")
...
def run_gpt_prompt_convo_to_thoughts(persona, 
  ...
  print ("PROMPT TEMPLATE: convo_to_thoughts_v1.txt")
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False): 
  ...
  def get_fail_safe(n): 
    return {f"I am hungry {i+1}": [] for i in range(n)}
  ...
  # example_output = "1. John Smith is determined to pass his Geology test (because of 1, 3, 5)\n"
  # example_output += "2. John Smith has a crush on Nancy Johnson"
  prompt_input = create_prompt_input(persona, statements, n)
  prompt = generate_prompt(prompt_input, prompt_template)
  ...
  if debug or verbose: 
    print("THOUGHTS GENERATED VERBOSE")
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
  ...
  print ("PROMPT TEMPLATE: agent_chat_v1.txt")
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  print ("PROMPT TEMPLATE: generate_next_convo_line_v1.txt")
  prompt_input = create_prompt_input(persona, interlocutor_desc, prev_convo, retrieved_summary)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  print ("PROMPT TEMPLATE: planning_thought_on_convo_v1.txt")
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  print ("PROMPT TEMPLATE: memo_on_convo_v1.txt")
  prompt_input = create_prompt_input(persona, all_utt)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  logging.debug("DEBUG:: Entering run_gpt_generate_iterative_chat_utt")
  ...
  print ("PROMPT TEMPLATE: iterative_convo_v1.txt")
