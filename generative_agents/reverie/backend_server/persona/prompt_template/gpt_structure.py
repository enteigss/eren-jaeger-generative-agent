...
def ChatGPT_safe_generate_response_OLD(prompt, 
                                   ...
                                   func_clean_up=None,
                                   verbose=False): 
  logging.debug("DEBUG:: Entering ChatGPT_safe_generate_response_OLD")
  logging.debug(f"DEBUG:: CHAT GPT PROMPT: {prompt}")
  logging.debug(f"DEBUG:: {prompt}")
  if verbose: 
    print ("CHAT GPT PROMPT")
    print (prompt)
...
def safe_generate_response(prompt, 
  ...
  try:
    ...
    if "token" in str(e).lower():
      ...
      try:
        import tiktoken
        enc = tiktoken.encoding_for_model(gpt_parameter["engine"])
        token_count = len(enc.encode(prompt))
        print(f"Approximate token count: {token_count}")
      except ImportError:
        print("tiktoken not installed - can't count tokens")
    raise
